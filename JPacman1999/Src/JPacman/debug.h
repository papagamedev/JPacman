...
#ifdef _DEBUG
    #define DPF dprintf
    #define ASSERT(x) if (! (x)) { DPF (DEBUG_ALWAYS, TEXT("Assertion violated: %s, File = %s, Line = #%ld\n"), TEXT(#x), TEXT(__FILE__), (DWORD)__LINE__ ); abort ();}        

   #define REPORTERR(x) ReportDDError ((x), TEXT("File = %s, Line = #%ld\n"), TEXT(__FILE__), (DWORD)__LINE__ );

   #define FATALERR(x) ReportDDError ((x), TEXT("File = %s, Line = #%ld\n"), TEXT(__FILE__), (DWORD)__LINE__ ); DestroyWindow (g_hMainWindow);
#else
   #define REPORTERR(x)
   #define DPF 1 ? (void)0 : (void)
   #define ASSERT(x)
   #define FATALERR(x)   DestroyWindow (g_hMainWindow);
#endif // DEBUG
